...
For two programmes, the questionnaire has been completed twice: 
- The MZPM programme
- Aerial surveys by HiDef

For the MZPM programme, we here keep the information provided by the `Chief Inspector of Environmental Protection` as this information is more inclusive and is confirmed by Dominik Marchowski. For the Arial surveys of HiDef, we keep the last record (4/11/2020) as this is more complete.

```{r}
input_data %<>% 
  filter(!timestamp == "11/4/2020 12:34:54") %>%  # Select HiDef programme on timestamp
  filter(!timestamp == "8/18/2020 8:45:54") # Select on MZPM timestamp
